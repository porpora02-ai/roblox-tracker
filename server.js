...
const MONGO_URI = "mongodb+srv://VantixSS:IMBACKLOL@vantixss.kyskpmm.mongodb.net/vantix?appName=VantixSS";
...
app.post("/api/signup", async (req, res) => {
    try {
        ...
        const user = new User({ email, username, password: hash, dob, tier: "none" });
        await user.save();

        req.session.username = username;
        res.json({ ok: true, username, tier: "none", isOwner: username === OWNER });
    } catch (e) {
        console.error(e);
        res.json({ ok: false, error: "Signup failed, try again" });
    }
});
...
app.post("/api/login", async (req, res) => {
    try {
        ...
        res.json({ ok: false, error: "Login failed, try again" });
    }
});
